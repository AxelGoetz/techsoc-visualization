...
},{"./lodash.js":20}],20:[function(require,module,exports){
(function (global){
/**
 * @license
...
var ACCESS_TOKEN = "CAAK3kpvZBViIBAOEZC6B1cXrA1HpCNSSz5WpTNpHUwWbSuSKlQKPtx3s28rKrSdk1ViWhk0f8g5XdppQAMsHl0FAhcKHYctj6CxiGC9BbG81jXoWIOYBOKftYB1KpLZCkBtQZCkbuo89JWoCmKazsAWih7MdMK59UbsPGHzYkP4ZAKZCjKXqTpowRVj3k9w14ZD";
...
function extractData(dataPoint) {
  var msec = Date.parse(dataPoint['Order date'].slice(0, 10));
  var d = new Date(msec);
...
function getMembersPerDay() {
  membersPerDay = _d32["default"].nest().key(function (d) {
    return d.date;
  }).sortKeys(_d32["default"].ascending).key(function (d) {
    return d.year;
  }).sortKeys(_d32["default"].ascending).rollup(function (leaves) {
    return leaves.length;
  }).entries(memberData);

  membersPerDay.forEach(function (e, i, array) {
    var membersJoined = 0;
    e.values.forEach(function (e2, i2, array2) {
...
function getMembersPerYear() {
  membersPerYear = _d32["default"].nest().key(function (d) {
    return d.year;
  }).sortKeys(_d32["default"].ascending).rollup(function (leaves) {
    return leaves.length;
  }).entries(memberData);
...
function extractDate(dataPoint) {
  var msec = Date.parse(dataPoint.start_time.slice(0, 10));
...
function filterOldandNewEvents() {
  var oldestTicket = new Date(membersPerDay[0].key).getTime();
  var newestTicket = new Date(membersPerDay[membersPerDay.length - 1].key).getTime();

  facebookData = _lodash2["default"].filter(facebookData, function (d) {
    var date = new Date(d.start_time).getTime();
...
function addMissingDaysFirstEvent() {
  while (facebookData[0].start_time.getTime() != new Date(membersPerDay[0].key).getTime()) {
    var prevDay = new Date(facebookData[0].start_time);
    prevDay.setDate(facebookData[0].start_time.getDate() - 1);
    var tempObj = { start_time: prevDay, id: "", attending_count: 0, interested_count: 0, name: "" };
...
function addMissingDaysFacebook() {
  ...
  var length = facebookData.length - 1;
  for (var i = 0; i < length; i++) {
    facebookData[i].start_time = new Date(facebookData[i].start_time);
    facebookData[i].start_time.setHours(1);
    var nextDay = new Date(facebookData[i].start_time);
    nextDay.setDate(facebookData[i].start_time.getDate() + 1);

    var nextDayObject = new Date(facebookData[i + 1].start_time);
    nextDayObject.setHours(1);
    if (facebookData[i].start_time.getTime() == nextDayObject.getTime()) {
      var amountPeople1 = facebookData[i].attending_count + facebookData[i].interested_count;
      var amountPeople2 = facebookData[i + 1].attending_count + facebookData[i + 1].interested_count;
      ...
      var tempObj = { start_time: nextDay, id: "", attending_count: 0, interested_count: 0, name: "" };
      facebookData.push(tempObj);
      facebookData.sort(sortFacebook);
      length++;
    }
...
function getFacebookEvents() {
  _axios2["default"].get("https://graph.facebook.com/v2.5/UCLUTechSoc/events", {
    params: {
      access_token: ACCESS_TOKEN,
      fields: ["name", "start_time", "attending_count", "interested_count"].join(","),
      limit: 500, // Assuming techsoc will never have more than 500 events :P
      json: true
    }
  }).then(function (response) {
    facebookData = response.data.data;
    _lodash2["default"].map(facebookData, extractDate);
...
_d32["default"].csv('./data/members.csv', function (error, data) {
  _lodash2["default"].map(data, extractData);
  getMembersPerDay();
  getMembersPerYear();
  getFacebookEvents();
});
...
_d32["default"].json('http://techsoc.io/data/events.json', function (error, data) {
  data = _lodash2["default"].filter(data, function (d) {
    return d.facebook_id !== undefined;
  });
  _lodash2["default"].map(data, getFacebookAttendees);
  eventsData = data;
});
...
function addNewSVG(bounds) {
  ...
  svg.attr("width", bounds.width + bounds.margin.left + bounds.margin.right).attr("height", bounds.height + bounds.margin.top + bounds.margin.bottom);

  return svg.append('g').attr("transform", "translate(" + bounds.margin.left + "," + bounds.margin.top + ")");
}
...
function getBounds() {
  ...
  var bound = { height: bounds.height, width: bounds.width, margin: { top: 50, right: 30, bottom: 30, left: 100 } };
...
function addAxisMemberBarChart(g, bounds, y) {
  var x = _d32["default"].scale.ordinal().rangeRoundBands([0, bounds.width], 0.1);

  var xAxis = _d32["default"].svg.axis().scale(x).orient("bottom");

  var yAxis = _d32["default"].svg.axis().scale(y).orient("left");

  g.append("g").attr("class", "x axis").attr("transform", "translate(0," + bounds.height / 2 + ")").call(xAxis);

  g.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").text("Members");
}
...
function addTextMemberBarChart(bar, bounds) {
  var text = bar.append("text").attr("x", 0).attr("y", 0).attr('transform', 'translate(' + -100 + ", " + 0 + ")").attr("display", "none");

  text.selectAll('tspan').data(function (d) {
    return d.values === null ? [] : d.values;
  }).enter().append('tspan').attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return "20" + d.key + ": " + d.values;
  });

  var totalSpan = text.insert('tspan', ":first-child");
  totalSpan.attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return "Total: " + d.membersJoined;
  });

  var date = text.insert('tspan', ":first-child");
  date.attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return String(d.key).slice(4, 15);
  });
}

// Adds the text on hover for facebook
function addTextFacebookBarChart(bar, bounds) {
  var text = bar.append("text").attr("x", 0).attr("y", 0).attr('transform', 'translate(' + -100 + ", " + (bounds.height - 100) + ")").attr("display", "none");

  text.append('tspan').attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return d.name;
  });

  text.append('tspan').attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return String(d.start_time).slice(4, 15);
  });

  text.append('tspan').attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return "Attending: " + d.attending_count;
  });

  text.append('tspan').attr('x', 0).attr('dy', '1.2em').text(function (d) {
    return "Interested: " + d.interested_count;
  });
}

// Function responsible for actually adding the member bars
function addMembersBars(bounds, g, barWidth) {
  var y = _d32["default"].scale.linear().range([bounds.height / 2, 0]);

  y.domain([0, _d32["default"].max(membersPerDay, function (d) {
    return d.membersJoined;
  })]);

  var bar = g.selectAll(".members").data(membersPerDay).enter().append("g").attr("transform", function (d, i) {
    return "translate(" + i * barWidth + ",0)";
  }).attr("class", "bar members").on('mouseover', function () {
    _d32["default"].select(this).select('text').attr('display', 'inline');
  }).on('mouseout', function () {
    _d32["default"].select(this).select('text').attr('display', 'none');
  });

  bar.append("rect").attr("width", barWidth - 1)
  ...
  // .duration(100)
  .attr("y", function (d) {
    return y(d.membersJoined);
  }).attr("height", function (d) {
    return bounds.height / 2 - y(d.membersJoined);
  });
...
function addEventsBars(bounds, g, barWidth) {
  var y = _d32["default"].scale.linear().range([bounds.height / 2, bounds.height]);

  y.domain([0, _d32["default"].max(facebookData, function (d) {
    return d.interested_count > d.attending_count ? d.interested_count : d.attending_count;
  })]);

  var bar = g.selectAll(".events").data(facebookData).enter().append("g").attr("transform", function (d, i) {
    return "translate(" + i * barWidth + "," + 0 + ")";
  }).attr("class", "bar facebook").on('mouseover', function () {
    _d32["default"].select(this).select('text').attr('display', 'inline');
  }).on('mouseout', function () {
    _d32["default"].select(this).select('text').attr('display', 'none');
  });

  bar.append("rect").attr('class', 'interested').attr('x', 0).attr("y", bounds.height / 2).attr("height", function (d) {
    return y(d.interested_count) - bounds.height / 2;
  }).attr("width", barWidth / 2 - 1);

  bar.append("rect").attr('class', 'attending').attr('x', barWidth / 2).attr("y", bounds.height / 2).attr("height", function (d) {
    return y(d.attending_count) - bounds.height / 2;
  }).attr("width", barWidth / 2 - 1);
...
function membersBarChart() {
  ...
  var g = addNewSVG(bounds);

  var barWidth = bounds.width / membersPerDay.length;
...
function totalMembers() {
  var members = 0;
  _lodash2["default"].map(membersPerDay, function (d) {
    return members += d.membersJoined;
  });
  return members;
}
...
function addAxisCumulativeChart(g, bounds, y, text) {
  ...
  g.append("g").attr("class", "x axis").attr("transform", "translate(0," + bounds.height + ")").call(xAxis);

  g.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").text(text);
}

// Adds the text information to the circle
// elements
function addTextCumulativeGraph(circles, bounds) {
  var text = circles.append("text").attr("x", 0).attr("y", 0).attr('transform', 'translate(' + -100 + ", " + -50 + ")").attr("display", "none");
...
function drawLines(g, bounds, y, linex) {
  var currentTotal = 0;
  var line = _d32["default"].svg.line().x(function (d, i) {
    return i * linex;
  }).y(function (d) {
  ...
  g.append('path').datum(membersPerDay).attr('class', 'line').attr('d', line);
}
...
function drawCircles(g, bounds, y, linex) {
  var currentTotal = 0;
  var circles = g.selectAll("g").data(membersPerDay).enter().append("g").attr('class', 'bar').attr("transform", function (d, i) {
    currentTotal += d.membersJoined;
  ...
  circles.append("circle").attr("cx", 0).attr("cy", 0).attr("r", 2);
...
function cumulativeLineChart() {
  ...
  var y = _d32["default"].scale.linear().range([bounds.height, 0]);
  y.domain([0, totalMembers()]);
  var linex = bounds.width / membersPerDay.length;
...
function drawYearBarChart(g, bounds, barWidth, y) {
  var bar = g.selectAll("g").data(membersPerYear).enter().append("g").attr("transform", function (d, i) {
    return "translate(" + i * barWidth + ",0)";
  }).attr("class", "bar year");

  bar.append("rect").attr("width", barWidth - 1).attr("y", function (d) {
    return y(d.values);
  }).attr("height", function (d) {
    return bounds.height - y(d.values);
  });

  var text = bar.append("text").attr("x", barWidth / 3).attr("y", function (d) {
    return y(d.values) - 40;
  });

  text.append('tspan').attr('x', barWidth / 3).attr('dy', '1.2em').text(function (d) {
    return '20' + d.key;
  });

  text.append('tspan').attr('x', barWidth / 4).attr('dy', '1.2em').text(function (d) {
...
function membersPerYearBarChart() {
  ...
  var y = _d32["default"].scale.linear().range([bounds.height, 0]);
  y.domain([0, _d32["default"].max(membersPerYear, function (d) {
    return d.values;
  })]);
  var barWidth = bounds.width / membersPerYear.length;
...
function getEventAttendees(event, URLString) {
  _axios2["default"].get("https://graph.facebook.com/v2.5/" + event.facebook_id + '/' + URLString, {
    params: {
      access_token: ACCESS_TOKEN,
      json: true
    }
...
function getFacebookAttendees(event) {
  if (event.facebook_id === undefined) {
    ...
    _axios2["default"].get("https://graph.facebook.com/v2.5/" + event.facebook_id, {
      params: {
        access_token: ACCESS_TOKEN,
        fields: ["attending_count", "interested_count"].join(","),
        json: true
      }
    }).then(function (response) {
...
function drawEventsBarChart(g, bounds, barWidth, y) {
  var bar = g.selectAll("g").data(eventsData).enter().append("g").attr("transform", function (d, i) {
    return "translate(" + i * barWidth + ",0)";
  }).attr("class", "bar event").on('mouseover', function () {
    _d32["default"].select(this).select('text').attr('display', 'inline');
  }).on('mouseout', function () {
    _d32["default"].select(this).select('text').attr('display', 'none');
  });

  bar.append("rect").attr("class", "attending").attr("width", barWidth - 1).attr("y", function (d, i) {
    return y(d.attending_count);
  }).attr("height", function (d) {
    return bounds.height - y(d.attending_count);
  });

  bar.append("rect").attr("class", "interested").attr("width", barWidth - 1).attr("y", function (d) {
    return y(d.interested_count) - (bounds.height - y(d.attending_count));
  }).attr("height", function (d) {
    return bounds.height - y(d.interested_count);
  });
...
function eventsBarChart() {
  var bounds = getBounds();
  var g = addNewSVG(bounds);
  eventsData = _lodash2["default"].filter(eventsData, function (d) {
    return d !== null && d.attending_count !== undefined && d.facebook_id !== undefined && d.interested_count !== undefined;
  });

  console.log(eventsData);
  var y = _d32["default"].scale.linear().range([bounds.height, 0]);
  y.domain([0, _d32["default"].max(eventsData, function (d) {
    return d.attending_count + d.interested_count;
  })]);
  var barWidth = bounds.width / eventsData.length;
